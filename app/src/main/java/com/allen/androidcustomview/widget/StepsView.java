...
import com.allen.androidcustomview.utils.WindowUtils;
...
public class StepsView extends LinearLayout implements StepsViewIndicator.OnDrawListener {
    ...
    public StepsView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        init();
        //获取初始化配置，属性
        TypedArray arr = context.obtainStyledAttributes(attrs, R.styleable.StepsViewss);
        int numSteps = arr.getInt(R.styleable.StepsViewss_numOfSteps, 3);
        int completePos = arr.getInt(R.styleable.StepsViewss_completePosition, 0);
        int labelsResId = arr.getResourceId(R.styleable.StepsViewss_labels, 0);
        int barColor = arr.getColor(R.styleable.StepsViewss_barColor, Color.GRAY);
        int progressColor = arr.getColor(R.styleable.StepsViewss_progressColorss, ContextCompat.getColor(context, R.color.orange));
        int labelColor = arr.getColor(R.styleable.StepsViewss_labelColor, Color.BLACK);
        int progressTextColor = arr.getColor(R.styleable.StepsViewss_progressTextColor, Color.WHITE);
        boolean hideProgressText = arr.getBoolean(R.styleable.StepsViewss_hideProgressText, false);
        float labelSize = arr.getFloat(R.styleable.StepsViewss_labelSize, 20);
        float progressMargin = arr.getFloat(R.styleable.StepsViewss_progressMargin, 100);
        float circleRadius = arr.getFloat(R.styleable.StepsViewss_circleRadius, 50);
        float progressStrokeWidth = arr.getFloat(R.styleable.StepsViewss_progressStrokeWidthss, 5);
    ...
    public void drawView() {
        ...
        //StepsView 只是一个容器，在容器中不断添加
        //mStepsViewIndicator  这个是圆，，通过invalidate的方式重新绘制
        mStepsViewIndicator.invalidate();
//        mStepsViewIndicator.setProgress(0, 80);
    }
    ...
    private void drawLabels() {
        ...
        if (mLabels != null) {
            for (int i = 1; i < mLabels.length; i++) {
                ...
                layoutParams.width = WindowUtils.dp2px(getContext(), 16);
                layoutParams.height = WindowUtils.dp2px(getContext(), 16);
                imageView.setLayoutParams(layoutParams);
                if (i == 1) {
                    imageView.setImageResource(R.mipmap.sign_icon_gift_selected); //图片资源
                } else {
                    imageView.setImageResource(R.mipmap.sign_icon_gift_normal); //图片资源
                }
                imageView.setX(indicatorPosition.get(i) - (textWidth / 2));
                mLabelsLayout.addView(imageView); //动态添加图片
